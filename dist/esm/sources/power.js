import { ABS, ARCTAN, ASSUME_REAL_VARIABLES, avoidCalculatingPowersIntoArctans, breakpoint, caddr, cadr, car, cdr, Constants, COS, defs, E, isadd, iscons, isdouble, ismultiply, isNumericAtom, ispower, isrational, istensor, LOG, MULTIPLY, PI, POWER, SIN } from '../runtime/defs.js';
...
import { compare_numbers, double, integer, nativeDouble, nativeInt, rational } from './bignum.js';
import { conjugate } from './conj.js';
import { cosine } from './cos.js';
import { dpow } from './dpow.js';
import { Eval } from './eval.js';
import { factorial } from './factorial.js';
import { iscomplexnumber, iscomplexnumberdouble, iseveninteger, isinteger, isminusone, isminusoneovertwo, isone, isoneovertwo, ispositivenumber, isquarterturn, isZeroAtomOrTensor } from './is.js';
...
function yypower(base, exponent) {
    ...
    if (ismultiply(base) && isinteger(exponent)) {
        ...
        if (iscons(base)) {
            result = base
                .tail()
                .reduce((a, b) => multiply(a, power(b, exponent)), result);
        }
        if (DEBUG_POWER) {
    ...
    if (isNumericAtom(cadr(base))) {
        is_a_moreThanZero =
            sign(compare_numbers(cadr(base), Constants.zero)) > 0;
    }
    if (ispower(base) && // when c is an integer
    ...
    if (defs.trigmode === 1 &&
        car(base) === symbol(SIN) &&
        iseveninteger(exponent)) {
        const result = power(subtract(Constants.one, power(cosine(cadr(base)), integer(2))), multiply(exponent, rational(1, 2)));
        if (DEBUG_POWER) {
    ...
    if (defs.trigmode === 2 &&
        car(base) === symbol(COS) &&
        iseveninteger(exponent)) {
        const result = power(subtract(Constants.one, power(sine(cadr(base)), integer(2))), multiply(exponent, rational(1, 2)));
        if (DEBUG_POWER) {
    ...
    if (iscomplexnumber(base)) {
        ...
        if (isNumericAtom(exponent)) {
            ...
            const pi = defs.evaluatingAsFloats ||
                ...
                ? double(Math.PI)
                : symbol(PI);
            let tmp = multiply(power(abs(base), exponent), power(Constants.negOne, divide(multiply(arg(base), exponent), pi)));
