...
import { breakpoint, car, cdr, Constants, defs, INNER, isadd, iscons, isinnerordot, isNumericAtom, isNumericAtomOrTensor, istensor, MAXDIM, NIL, SYMBOL_IDENTITY_MATRIX } from '../runtime/defs.js';
    ...
    if (operands.length > 1) {
        ...
        for (let i = 0; i < operands.length; i++) {
            ...
            if (i + shift + 1 <= operands.length - 1) {
                ...
                if (!(isNumericAtomOrTensor(operands[i + shift]) ||
...
export function inner(p1, p2) {
    ...
    if (istensor(p1) && istensor(p2)) {
        ...
        if (defs.expanding && isadd(p1)) {
            return p1
                .tail()
                .reduce((a, b) => add(a, inner(b, p2)), Constants.zero);
        }
        if (defs.expanding && isadd(p2)) {
            return p2
                .tail()
                .reduce((a, b) => add(a, inner(p1, b)), Constants.zero);
        }
