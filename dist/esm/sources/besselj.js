import { BESSELJ, caddr, cadr, Constants, defs, isdouble, MEQUAL, MSIGN, NUM, PI } from '../runtime/defs.js';
...
*/
export function Eval_besselj(p1) {
    return besselj(Eval(cadr(p1)), Eval(caddr(p1)));
}
export function besselj(p1, p2) {
...
function yybesselj(X, N) {
    ...
    if (N.k === NUM && MEQUAL(N.q.b, 2)) {
        ...
        return subtract(multiply(multiply(divide(integer(2), X), subtract(N, SGN)), besselj(X, subtract(N, SGN))), besselj(X, subtract(N, multiply(integer(2), SGN))));
    }
    //if 0 # test cases needed
    if (isnegativeterm(X)) {
        return multiply(multiply(power(negate(X), N), power(X, negate(N))), makeList(symbol(BESSELJ), negate(X), N));
    }
    if (isnegativeterm(N)) {
