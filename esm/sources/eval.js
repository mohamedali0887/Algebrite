...
import { breakpoint, caadr, cadadr, cadddr, caddr, cadr, car, cdadr, cddr, cdr, CONS, Constants, DEBUG, defs, DOUBLE, EVAL, INDEX, iscons, isdouble, ISINTEGER, isNumericAtom, isrational, issymbol, istensor, LAST, NIL, NUM, OPERATOR, PI, SETQ, STR, SYM, TENSOR, TESTEQ } from '../runtime/defs.js';
...
export function evaluate_integer(p) {
...
export function Eval_binding(p1) {
...
export function Eval_divisors(p1) {
...
export function Eval_dsolve(p1) {
...
export function Eval_exp(p1) {
    return exponential(Eval(cadr(p1)));
}
export function Eval_factorial(p1) {
    return factorial(Eval(cadr(p1)));
}
export function Eval_factorpoly(p1) {
    p1 = cdr(p1);
...
export function Eval_inv(p1) {
    const arg = Eval(cadr(p1));
    return inv(arg);
}
export function Eval_invg(p1) {
    const arg = Eval(cadr(p1));
    return invg(arg);
}
export function Eval_isinteger(p1) {
    p1 = Eval(cadr(p1));
    if (isrational(p1)) {
...
export function Eval_number(p1) {
    p1 = Eval(cadr(p1));
...
export function Eval_operator(p1) {
    return makeList(symbol(OPERATOR), ...evalList(cdr(p1)));
}
// quote definition
export function Eval_quote(p1) {
    return cadr(p1);
}
// rank definition
export function Eval_rank(p1) {
    p1 = Eval(cadr(p1));
    if (istensor(p1)) {
...
export function Eval_predicate(p1) {
