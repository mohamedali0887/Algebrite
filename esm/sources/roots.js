...
import { caddr, cadr, car, Constants, DEBUG, defs, ismultiply, ispower, NIL, SECRETX, SETQ, TESTEQ } from '../runtime/defs.js';
import { stop } from '../runtime/run.js';
import { symbol } from '../runtime/symbol.js';
import { sort } from '../sources/misc.js';
...
//define C p5
//define Y p6
export function Eval_roots(POLY) {
    // A == B -> A - B
    let X = cadr(POLY);
    let POLY1;
...
function _solveDegree2(A, B, C) {
    ...
    const p6 = power(
    // prettier-ignore
    subtract(power(B, integer(2)), multiply(multiply(integer(4), A), C)), rational(1, 2));
    // ((B^2 - 4AC)^(1/2) - B)/ (2A)
...
function _solveDegree4Biquadratic(A, B, C, D, E) {
    ...
    const biquadraticSolutions = roots(add(multiply(A, power(symbol(SECRETX), integer(2))), add(multiply(C, symbol(SECRETX)), E)), symbol(SECRETX));
    const results = [];
...
function _solveDegree4NonzeroB(A, B, C, D, E) {
    ...
    let R_r = divide(add(multiply(power(B, integer(4)), integer(-3)), add(multiply(integer(256), multiply(R_a3, E)), add(multiply(integer(-64), multiply(R_a2_d, B)), multiply(integer(16), multiply(R_b2, multiply(A, C)))))), multiply(integer(256), power(A, integer(4))));
