import { cadr, Constants, isadd, noexpand } from '../runtime/defs.js';
...
export function Eval_condense(p1) {
    return Condense(Eval(cadr(p1)));
}
export function Condense(p1) {
    return noexpand(yycondense, p1);
}
export function yycondense(p1) {
    //expanding = 0
    if (!isadd(p1)) {
        return p1;
    }
    ...
    const temp2 = p1
        .tail()
        .reduce((a, b) => add(a, multiply_noexpand(p2, b)), Constants.zero);
