...
import { ADD, caddr, cadr, car, cdr, Constants, COS, DEBUG, defs, do_simplify_nested_radicals, FACTORIAL, FUNCTION, INTEGRAL, isadd, iscons, isinnerordot, ismultiply, ispower, istensor, MAX_CONSECUTIVE_APPLICATIONS_OF_ALL_RULES, MAX_CONSECUTIVE_APPLICATIONS_OF_SINGLE_RULE, METAA, METAB, METAX, MULTIPLY, NIL, noexpand, POWER, SECRETX, SIN, TRANSPOSE, } from '../runtime/defs.js';
...
import { equalq, isfraction, isimaginaryunit, isminusone, isnegativenumber, isone, isZeroAtomOrTensor, } from './is.js';
import { makeList } from './list.js';
import { equal, length } from './misc.js';
import { divide, inverse, multiply, multiply_noexpand, negate } from './multiply.js';
...
export function Eval_simplify(p1) {
...
function runUserDefinedSimplifications(p) {
    ...
    // recursively?
    if (defs.userSimplificationsInListForm.length === 0 ||
...
// ------------------------
export function simplifyForCodeGeneration(p) {
...
function f10(p1) {
    ...
    if (carp1 === symbol(MULTIPLY) || isinnerordot(p1)) {
        ...
        if (car(car(cdr(p1))) === symbol(TRANSPOSE) &&
...
export function simplify_trig(p1) {
    return f5(p1);
}
function f5(p1) {
