import { caddr, cadr, car, cdr, Constants, doexpand, isadd, iscons, ismultiply, isNumericAtom, ispower, isrational, MULTIPLY } from '../runtime/defs.js';
...
export function areunivarpolysfactoredorexpandedform(p1, p2) {
    let polyVar;
...
function gcd_term_term(p1, p2) {
    ...
    return p1.tail().reduce((a, b) => {
        return p2.tail().reduce((x, y) => multiply(x, gcd(b, y)), a);
    }, Constants.one);
}
function gcd_sum_product(p1, p2) {
    return iscons(p1)
        ? p1.tail().reduce((a, b) => multiply(a, gcd(b, p2)), Constants.one)
        : Constants.one;
}
function gcd_product_sum(p1, p2) {
    return iscons(p2)
        ? p2.tail().reduce((a, b) => multiply(a, gcd(p1, b)), Constants.one)
        : Constants.one;
}
